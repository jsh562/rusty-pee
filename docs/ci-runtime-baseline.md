# rusty-pee — CI Runtime Baseline

**Status**: v0.2.0 baseline projection (E011 Phase 6 T075).
...
v0.2.0 and rusty-vipe v0.2.0 baselines (also zero-leaf single-capability
ports).

The full v0.2.0 CI matrix for rusty-pee has these jobs:

| Tier | Job name | OS | Count | Notes |
|------|----------|-----|------:|-------|
| Pre-gates | fmt, clippy, audit, msrv | Linux | 4 | Identical to v0.1.x. |
| Tier 1 | test-default | linux x86_64 + aarch64 + macos x86_64 + macos aarch64 + windows x86_64 | 5 | Full DDR-003 cross-compile matrix (preserves v0.1.x coverage). The Linux x86_64 entry also runs the SC-002 default-install smoke (drives the binary through a fake-pee-child for non-interactive validation). |
| Tier 2 | test-no-default | Linux x86_64 | 1 | Bare library + SC-001 dep-tree audit. |
| Tier 3 | test-pee-classic, test-pee-minimal | Linux x86_64 | 2 | Preset bundle install + SC-003 size check. |
| Tier 3 (SC-004) | test-keeplist | Linux x86_64 | 1 | Keep-list workaround install smoke (fake-pee-child driven). |
...
GitHub-hosted matrix). rusty-pee has signal-injection / no-TTY
fault-injection tests in `tests/signal_cleanup.rs` that are
`#[ignore]`'d at v0.1.0, so the default CI test pass does not exercise
them.
